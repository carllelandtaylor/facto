...
Repeat the following cycle. Stop when a review cycle produces no feedback.
...
If the review returned no feedback items, the loop is done. Proceed to the "Done" section below.
...
If validation fails, fix the failure and amend into the appropriate commit. Do not proceed to the next review cycle until validation passes.
...
- **Maximum cycles.** The caller may specify a maximum number of cycles (default: 5). If you've hit the limit and there's still feedback, stop and report the remaining items to the user. Something may need human judgment.
...
2. Report:
   ...
   - Any items you chose not to address and why
