...
2. Flow map — Create a flowchart to describe navigation, where each node is one view involved in this feature, and each edge represents a possible transition between views. For each transition, note the trigger (user action, API response, timer, etc.).
...
Create a subagent. Give it the same inputs you have, plus what you created in the previous phase. Instruct the subagent to do an adversarial review of the flow map and view inventory you created in Phase 3. The subagent's goal is to find any possible flaw, gap, inefficiency, or non-obvious edge case in the flow map and view inventory. The subagent should be critical and try to break the flow or find problems for users.
...
3. The URLs for the two output files you created.
